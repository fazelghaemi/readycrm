...
// اضافه کردن ووکامرس
<li><a href="#woocommerce" class="tab-link">🛒 WooCommerce</a></li>
<!-- WooCommerce Settings -->
<div id="woocommerce" class="tab-content">
    <h2>تنظیمات WooCommerce</h2>
    
    <form method="POST" action="settings.php?action=save_woo">
        <div class="form-group">
            <label>
                <input type="checkbox" name="woo_enabled" value="1" <?= WOO_ENABLED ? 'checked' : '' ?>>
                فعال‌سازی ادغام WooCommerce
            </label>
        </div>

        <div class="form-group">
            <label>آدرس فروشگاه:</label>
            <input type="url" name="woo_store_url" value="<?= htmlspecialchars(WOO_STORE_URL) ?>" 
                   placeholder="https://shop.example.com" class="form-control">
            <small>بدون / در انتها</small>
        </div>

        <div class="form-group">
            <label>Consumer Key:</label>
            <input type="text" name="woo_consumer_key" value="<?= htmlspecialchars(WOO_CONSUMER_KEY) ?>" 
                   placeholder="ck_xxxxxxxxxxxxx" class="form-control">
        </div>

        <div class="form-group">
            <label>Consumer Secret:</label>
            <input type="password" name="woo_consumer_secret" value="<?= htmlspecialchars(WOO_CONSUMER_SECRET) ?>" 
                   placeholder="cs_xxxxxxxxxxxxx" class="form-control">
        </div>

        <div class="form-group">
            <label>
                <input type="checkbox" name="woo_verify_ssl" value="1" <?= WOO_VERIFY_SSL ? 'checked' : '' ?>>
                تایید گواهی SSL (برای localhost غیرفعال کنید)
            </label>
        </div>

        <button type="submit" class="btn btn-primary">💾 ذخیره تنظیمات</button>
        <button type="button" id="test-connection" class="btn btn-secondary">🔌 تست اتصال</button>
    </form>
    ...
    <div class="import-actions">
        <button id="import-products" class="btn btn-info">📦 Import محصولات</button>
        <button id="import-customers" class="btn btn-info">👥 Import مشتری‌ها</button>
        <button id="import-orders" class="btn btn-info">🛒 Import سفارش‌ها</button>
    </div>
    ...
    <div id="import-progress" style="display:none; margin-top:20px;">
        ...
        <p id="import-status">در حال پردازش...</p>
    </div>
