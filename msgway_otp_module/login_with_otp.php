<?php
// login_with_otp.php (MSGway OTP)
session_start();

$global = __DIR__ . '/../includes/security_bootstrap.php';
$module = __DIR__ . '/includes/module_bootstrap.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['send'])) {
        if (!$client) {
            $error = $error ?: 'کلاینت MSGway در دسترس نیست.';
        } else {
...
<div class="container">
  ...
  <form method="post">
    ...
    <input type="tel" name="mobile" inputmode="numeric" placeholder="09xxxxxxxxx"
           value="<?= htmlspecialchars($_POST['mobile'] ?? ($_SESSION['otp_mobile'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" required>
  ...
  <div class="hint">پس از تایید، مستقیماً وارد داشبورد می‌شوی.</div>
</div>
</body>
</html>
